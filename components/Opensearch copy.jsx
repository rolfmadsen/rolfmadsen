...
function useOpenPlatformSearch(queryString, start, stepValue) {
  ...
  React.useEffect(() => {
    async function fetchBookList() {
      try {
        setLoading(true);
        const response = await fetch(`/api/opensearch/search?searchquery=${validCql}&start=${start}&stepValue=${stepValue}`, {
          method: 'GET',
          headers: {
...
function Opensearch() {
  ...
  const [searchResult, loading] = useOpenPlatformSearch(queryString, '1', '10');
