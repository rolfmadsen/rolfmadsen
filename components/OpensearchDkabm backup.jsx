    ...
    <div>
      ...
      {loading === "false" ? (
          <div className="block relative border-solid border-2 border-gray-600 clear-both py-2 px-2 mx-4 my-4">
            ...
            <p className="block">This page is under active development.</p>
          </div>
        ) : loading === "null" ? (
          <div>
          ...
          SearchResult.searchResponse.result.searchResult[0].map(workCollection => {
            <>
              <div>
                <span>{workCollection.collection.object[0].record.title[0].$}</span>
                {/*
                <span title="hitcount" className="px-4 pb-4 inline-block">"{queryString}" gav {SearchResult.hitCount.$} resultater!</span>
                */}
              </div>
            </>
          })
